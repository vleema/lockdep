...
O mecanismo mais comum para interposição de funções em tempo de execução é através da variável de ambiente \texttt{LD\_PRELOAD}. Esta variável permite especificar bibliotecas compartilhadas que serão carregadas antes de quaisquer outras, dando a elas prioridade na resolução de símbolos.

O processo funciona da seguinte forma:

\begin{enumerate}
    \item O loader dinâmico (\texttt{ld.so} ou \texttt{ld-linux.so}) verifica a variável \texttt{LD\_PRELOAD}
...
O GCC e outros compiladores C suportam os atributos \texttt{\_\_attribute\_\_((constructor))} e \texttt{\_\_attribute\_\_((destructor))}, que permitem que funções sejam executadas automaticamente quando uma biblioteca compartilhada é carregada ou descarregada:
